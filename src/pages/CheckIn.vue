<template>
  <q-page class="container">
    ...
    <q-stepper ref="stepper" v-model="step" vertical color="primary" class="q-pa-md">
      <!-- 入住流程表单 -->
      <q-step default name="info" title="提供身份证" :done="step > 'info'">
        ...
        <q-btn @click="next" :disable="!id || !name" color="primary" label="下一步" icon-right="arrow" />
      </q-step>
      <q-step name="deposit" title="收取押金" :done="step > 'deposit'">
        <q-input filled v-model="deposit" label="押金金额：" />
        <q-btn @click="next" :disable="!deposit" color="primary" label="下一步" icon-right="arrow_forward" />
      </q-step>
      <q-step name="room" title="分配房间" :done="step > 'room'">
        ...
        <q-btn @click="submit" color="primary" label="完成入住" icon-right="done" />
      </q-step>
    </q-stepper>
